...
const MemberAttendanceList = (props) => {
    ...
    const onChangedropdown = (e) => {
        ...
        setListInput({ ...ListInput, ["member_id"]: e.value });
    };
    ...
    const LoadAllMemberLists = async (e) => {
        e.preventDefault();
        setWarnings({ ["warning"]: "" });
        ...
        await axios.post(process.env.REACT_APP_API + "AttendanceListOfMember", list, config).then(response => {
            console.log(response);
            if (response.data.is_success) {
                ...
                if (response.data.batches_list !== null) {
                    ...
                    for (let i = 0; i < response.data.attendance_member_list.length; i++) {
                        if (response.data.attendance_member_list[i].att_login_date !== null) {
                            var date = response.data.attendance_member_list[i].att_login_date;
                        ...
                        img = process.env.REACT_APP_PHOTOPATH + cookies.unique_code + "/CUSTOMER/" + response.data.attendance_member_list[i].customer_photo;
                        MemList.push({
                            "att_id": response.data.attendance_member_list[i].att_id,
                            "att_branch_id": response.data.attendance_member_list[i].att_branch_id,
                            "branch_name": response.data.attendance_member_list[i].branch_name,
                            "att_member_id": response.data.attendance_member_list[i].att_member_id,
                            "member_customer_id": response.data.attendance_member_list[i].member_customer_id,
                            "member_name": response.data.attendance_member_list[i].member_name,

                            "customer_photo": response.data.attendance_member_list[i].customer_photo,
                            "att_login_date": newdate,
                            "att_login_time": response.data.attendance_member_list[i].att_login_time,
                    ...
                    //setMemberLists(response.data.batches_list)
    ...
    return (
        <>
            {
                cookies.allowed_features.includes(AppConstants.ALLOWED_FEATURE_MEMBER_ATTENDANCE) ?
                    <div style={{ paddingRight: "30px", paddingLeft: "30px", paddingTop: "20px" }}>
                        <CRow>
                            <CCol xs="12" sm="12" ms="12" lg="12">
                                <CCard style={{ borderRadius: "20px" }}>
                                    <CCardBody>
                                        <h4>Member Attendance Report</h4>
                                        ...
                                        <CRow>
                                            ...
                                            <CCol xs="12" sm="3" md="3" lg="3" className="">
                                                <CFormGroup>
                                                    ...
                                                    <Select
                                                        ...
                                                        onChange={(e) => onChangedropdown(e, "member_id")}
                                                        required="required">
                                                    </Select>
                                                </CFormGroup>
                                            </CCol>
                                            ...
                                            <CCol xs="12" sm="3" md="3" lg="3" className="">
                                                <div className="bgcolor mt-3" style={{ borderRadius: "5px" }}>
                                                    <CButton type="submit" onClick={(e) => LoadAllMemberLists(e)} disabled={disablebutton} style={{ width: "100%", color: "white", fontWeight: "50%", fontSize: "15px" }} >Generate</CButton>
