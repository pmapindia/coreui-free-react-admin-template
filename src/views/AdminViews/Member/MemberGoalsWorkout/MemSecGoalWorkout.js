...
const MemSecGoalWorkOut = (props) => {
    ...
    const [SecGoalDropdowns, setSecGoalDropdowns] = useState([])
    ...
    const onChangeSecdropdown = (e) => {
        console.warn(e.value);//here i will get the specific selected value
        console.warn(e.label);
        setMemWorkoutAdd({ ...MemWorkoutAdd, ["mem_wc_map_secondary_goal_id"]: e.value });
    }
    ...
    const [WorkoutMapList, setSecWorkoutList] = useState([])

    const LoadWorkoutMapLists = async () => {
        setSecWorkoutList([])
        var list = {};
        list["search_text"] = search_text;
        await axios.post(process.env.REACT_APP_API + "MemberWorkoutListForSecondaryGoal", list, config).then(response => {
            console.log(response);
            if (response.data.is_success) {
                if (response.data.member_workout_seconary_goal_list !== null) {
                    setSecWorkoutList(response.data.member_workout_seconary_goal_list)
                }
                else {
                    setSecWorkoutList([])
                }
                                ...
                                <CCardBody>
                                    ...
                                    <CRow>
                                        ...
                                        <CCol xs="12" sm="3" md="3" lg="3">
                                            <CFormGroup>
                                                <CLabel>Select Secondary Goal</CLabel><span className="red">*</span>
                                                <Select value={SecGoalDropdowns.filter(function (option) {
                                                    return option.value === mem_wc_map_secondary_goal_id;
                                                })}
                                                    options={SecGoalDropdowns}
                                                    onChange={(e) => onChangeSecdropdown(e, "mem_wc_map_secondary_goal_id")} >
                                                </Select>
                                            </CFormGroup>
                                        </CCol>
                                ...
                                <CCardBody>
                                    <CRow>
                                        <CCol xs="12" sm="9" md="9" lg="9">
                                            <h5>Secondary Goal Workout List</h5>
                                        </CCol>
...
export default MemSecGoalWorkOut;