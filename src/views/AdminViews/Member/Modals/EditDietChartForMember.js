...
const EditDietChartForMember = (props) => {
    ...
    const [disablebuttonde, setDisableButtonDe] = useState(false);

    const OnSubmitDietchartUpdate = async (e) => {
        e.preventDefault();
        setDisableButtonDe(true);
        document.getElementById("img_gif_loading_btnde").style.display = "block";
        await axios.post(process.env.REACT_APP_API + "MemberDietChartUpdate", {
            "diet_chart_id": diet_chart_id,
            ...
            if (response.data.is_success) {
                ...
                setWarnings({ ["warning"]: "" });
                setDisableButtonDe(false);
                document.getElementById("img_gif_loading_btnde").style.display = "none";
    ...
    const MemDietCahrtDetailsByID = async () => {
        var list = {};
        list["diet_chart_id"] = props.dc_id;
        await axios.post(process.env.REACT_APP_API + "MemberDietChartDetailsByID", list, config).then(response => {
            console.log(response);
            if (response.data.is_success) {
                setMemDietChartAdd(response.data.member_diet_chart_details)
            }
            else {
            }
        }).catch(
    ...
    return (
        <CRow>
            <CCol>
                <button class='btn btn-sm btn-outline-primary' onClick={(e) => handleShow(e)}>Edit</button>
                ...
                >
                    ...
                    <CModalHeader style={{ backgroundColor: "#3b3f4b", borderColor: "#3b3f4b" }}>
                        ...
                        <CModalTitle className=" white mt-2"><h4>Update Diet Chart</h4></CModalTitle>
                    ...
                    <CModalBody>
                        <CForm onSubmit={(e) => OnSubmitDietchartUpdate(e)}>
                            <CRow>
                            ...
                            <CRow>
                                ...
                                <CCol xs="12" sm="12" md="12" lg="12" className="">
                                    <div className="bgcolor mt-1" style={{ borderRadius: "5px" }}>
                                        <CButton type="submit" disabled={disablebuttonde} style={{ width: "100%", color: "white", fontWeight: "50%", fontSize: "15px" }} >Save</CButton>
                                        <img id="img_gif_loading_btnde" src={process.env.PUBLIC_URL + '/avatars/gif_loading.gif'} style={{ width: "20px", marginTop: "-26px", float: "right", marginRight: "10px", display: "none" }} />
                                    </div>
...
export default EditDietChartForMember;