...
import MemSecGoalWorkOutAddtModal from './MemSecGoalWorkOutAddModal';

const MemSecGoalWorkOutListModal = (props) => {
    ...
    const handleShow = (e) => {
        ...
        setPrimary(!primary)
        setShow(true);
        WorkoutListBySecondaryGoalAndWeek(props.week_name);
    }

    const [WorkoutSGList, setWorkoutSGList] = useState([]);

    const WorkoutListBySecondaryGoalAndWeek = async (week_name) => {
        var list = {};
        list["member_id"] = props.mem_id;
        list["secondary_goal_id"] = props.sg_id;
        list["week_name"] = props.week_name;
        await axios.post(process.env.REACT_APP_API + "MemberWorkoutListBySecondaryGoalAndWeek", list, config).then(response => {
            console.log(response);
            if (response.data.is_success) {
                if (response.data.member_workout_secondary_goal_list.length !== null) {
                    setWorkoutSGList(response.data.member_workout_secondary_goal_list)
                }
                else {
                    setWorkoutSGList([]);
                }
    ...
    const WorkoutDelete = async (taber_index, wo_id) => {
        ...
        await axios.post(process.env.REACT_APP_API + "MemberWorkoutDelete", list, config).then(response => {
            ...
            if (response.data.is_success) {
                ...
                var temp_taber = WorkoutSGList;
                temp_taber.splice(taber_index, 1);
                setWorkoutSGList([]);
                setWorkoutSGList(temp_taber);
            ...
            <CCol>
                ...
                >
                    ...
                    <CModalHeader style={{ backgroundColor: "#3b3f4b", borderColor: "#3b3f4b" }}>
                        ...
                        <CModalTitle className=" white mt-2"><h4>Workout List for Secondary Goal</h4></CModalTitle>
                    ...
                    <CModalBody>
                        <CRow className=''>
                            ...
                            <CCol xs="10" sm="2" md="2" lg="2">
                                <MemSecGoalWorkOutAddtModal
                                    mem_id={props.mem_id}
                                    sg_id={props.sg_id}
...
export default MemSecGoalWorkOutListModal;