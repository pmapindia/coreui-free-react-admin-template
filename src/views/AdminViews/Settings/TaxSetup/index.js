...
const TaxSetup = (props) => {
    ...
    const OnSubmitTaxSetup = async (e) => {
        ...
        if (tax_id === "") {
            ...
            await axios.post(process.env.REACT_APP_API + "TaxSetupUpdate", {
                "tax_id": tax_id,
                "tax_name": tax_name,
                "tax_percentage": tax_percentage
            }, config).then(response => {
    ...
    return (
        <>
            {
                cookies.allowed_features.includes(AppConstants.ALLOWED_FEATURE_TAX_ADD) ?
