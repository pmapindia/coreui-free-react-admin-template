...
const TaxGroup = (props) => {
            ...
            if (response.data.is_success) {
                ...
                for (let i = 0; i < response.data.tax_group_setup_list.length; i++) {
                    if (response.data.tax_group_setup_list[i].tax_group_created_at !== null) {
                        ...
                        var cdatenew = cdate.substring(0, 10).substring(0, 10).split('-').reverse().join('-');
                    } else {
    ...
    return (
        <>
            {
                cookies.allowed_features.includes(AppConstants.ALLOWED_FEATURE_TAX_GROUP_ADD) ?
                                                ...
                                                <tbody>
                                                    {allTaxGroup.map((allTax1, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                <CButton className="btn" style={{ paddingLeft: "3px", alignSelf: "center" }} onClick={() => TaxGroupSetupDetailsByID(allTax1.tax_group_id)}>
