...
const BranchDetails = () => {
            ...
            if (response.data.is_success) {
                ...
                if (response.data.branch_details !== null) {
                    ...
                    if (response.data.branch_details.branch_created_at !== null) {
                        ...
                        CreateAt = date.substring(0, 10).split('-').reverse().join('-');
                    }
                    else {
    ...
    return (
        <>
            {
                cookies.allowed_features.includes(AppConstants.ALLOWED_FEATURE_BRANCH_DETAILS) ?
                                    ...
                                    <CCardBody>
                                        <CRow>
                                            <CCol xs="12" sm="10" md="10" lg="10">
                                                <h4 style={{ padding: "0px", margin: "0px" }}>Branch Details</h4>
                                            </CCol>

                                            <CCol xs="12" sm="2" md="2" lg="2">
                                                <span style={{ float: "", padding: "0px", margin: "0px" }}>
                                                    <Link
                                                        to={`generate-qr-code?branchid=${BranchDetails.branch_id}&&branchname=${BranchDetails.branch_name}&&orgname=${BranchDetails.branch_organization_name}`}
                                                        className="btn btn-sm bgcolor white width100">Generate QR Code</Link>
                                                </span>
                                            </CCol>
                                        </CRow>
